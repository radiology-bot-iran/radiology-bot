...
cursor.execute("SELECT * FROM events") rows = cursor.fetchall() if not rows: await update.message.reply_text("📭 فعلاً رویدادی وجود ندارد") return text = "📅 لیست رویدادها:\n\n" for row in rows: text += f"🎓 {row[1]}\n📆 {row[2]}\n👥 ظرفیت: {row[3]}\n\n" await update.message.reply_text(text) 
---------------- ADMIN PANEL ----------------
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
if not is_admin(update): await update.message.reply_text("⛔ دسترسی ندارید") return await update.message.reply_text( """ 
🛠 پنل مدیریت انجمن علمی
📅 /events 👥 /users 💬 /view_suggestions
🚧 امکانات در حال توسعه: ➕ افزودن رویداد ✏️ ویرایش رویداد 🗑 حذف رویداد 📚 مدیریت جزوات 🧪 مدیریت آزمون‌ها """ )
---------------- USERS ----------------
